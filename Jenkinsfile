		...
		stage ("Deploy - prod") {
            steps{
                sshagent(credentials : ['ssh-prod']) {
					...
					sh "ssh ubuntu@${IP_DEPLOY} docker stop \$(docker ps -a -q) || true"
                    sh "ssh ubuntu@${IP_DEPLOY} docker rm \$(docker ps -a -q) || true"
	                sh "ssh ubuntu@${IP_DEPLOY} docker rmi \$(docker images -a -q) || true"
